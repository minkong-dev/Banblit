"""정해진 시각이 지난 기간의 배정을 스스로 돌린다.

서버(api)와 같은 이미지를 쓰는 별도 컨테이너로 돈다. 서버를 여러 대로 늘려도
이 서비스만 한 대로 두면 같은 계산이 여러 번 돌지 않는다. HTTP 통로를 거치지 않고
...
class AutoRun:
    """자동 배정 한 기간분의 결과. slots 는 이번에 돈 것으로 표시한 연산 시각들이다."""
...
def due_slots(
    ...
    # period 의 두 연산 시각 중 now 까지 지났고 아직 안 돈 것을 돌려준다.
    # 어제 놓친 것은 보지 않는다 — 배정은 기간 전체를 다시 푸는 계산이라 어제 것을
    # 지금 돌리는 것과 오늘 것을 지금 돌리는 것이 같은 결과가 된다.
    today = now.date()
...
def run_due_assignments(session: Session, now: datetime) -> list[AutoRun]:
    """now 기준으로 돌 차례가 된 기간을 전부 계산하고, 돈 시각을 표에 남긴다.

    한 기간이 터져도 다음 기간을 계속 본다 — 실패는 기록에 남기고 표시는 남기지
    않아 다음 확인 때 다시 시도된다. 시각을 인자로 받으므로 검사가 아무 시각이나
    넣어볼 수 있다.
    """
    today = now.date()
    # 오늘이 기간 안에 드는 집중 합주기간만 본다. 상시 개방기간(open)은 선착순
    # 예약으로 돌아가 assign_period 가 아예 거절한다.
    ...
    # 사람이 없다. 등록된 전부를 대상으로 돈다 — 화면이 보내던 목록을 대신하는
    # 기본값이라, 일부만 돌리려면 그 목록을 남기는 표(period_rooms 등)가 먼저 있어야 한다.
...
def _run_one(
    ...
    # 푸는 것이라 결과가 같다. 늦은 쪽을 돌리고 이른 쪽도 돈 것으로 함께 표시한다.
    try:
    ...
    # 영영 안 돈다 — 그날 배정이 통째로 비는 쪽이, 22초짜리 계산을 한 번 더 도는
    # 쪽보다 나쁘다. 다시 돌아도 assign_period 는 같은 기간을 다시 풀어 덮어쓴다.
    _mark_ran(session, period_id, today, slots)

    # 알림은 저장이 실제로 된 뒤에만, 그리고 돈 표시를 남긴 뒤에 남긴다. 자리를 못
    # 찾아 저장이 안 되면 사람이 보던 시간표가 그대로라 알릴 것이 없고, 알리는 쪽이
    # 터져도 돈 표시는 이미 커밋돼 있어 22초짜리 계산을 다시 돌지 않는다.
    if result.saved:
...
def interval_seconds_from_env() -> int:
    ...
    # 기본값 — 0 이하를 그대로 쓰면 쉬지 않고 도는 반복이 된다.
    try:
...
def main() -> None:
    """확인 간격마다 깨어나 돌 것이 있는지 보는 반복. 컨테이너의 시작 명령이다."""
    ...
    while True:
        # 한 스레드에서 순서대로 돈다 — 계산(최대 22.2초, 2026-08-28 실측)이 확인
        ...
        except Exception:  # noqa: BLE001 - 저장소가 잠깐 끊겨도 다음 확인 때 다시 본다
