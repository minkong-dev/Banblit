...
# 한글은 영문·숫자가 아니라 특수기호 규칙([^A-Za-z0-9])을 충족해 버립니다. 그래서 규칙 4가지를

# 모두 통과하는 한글 비밀번호가 만들어집니다. 메일 서버와 다른 기기의 자판이 한글을 그대로

# 받아 주지 않아, 설정한 본인이 다시 입력하지 못하는 경우가 생깁니다.

@pytest.mark.parametrize("value", ["가나다라마바사아Abc1!", "Abcde1!가", "Ａbcdef1!", "Abcde1!\x00"])

def test_require_password_rejects_characters_outside_ascii(value: str) -> None:
...
# 제어문자는 화면에서 입력할 수 없지만 API 를 직접 호출하면 들어옵니다. 그대로 저장되면

# 메일 헤더·DB·기록에 보이지 않는 글자가 남습니다.

@pytest.mark.parametrize(

    "value", ["한글@example.com", "name@한글.com", "ａ@example.com", "a\x00b@example.com", "a\x1bb@example.com"]

)

def test_require_email_rejects_characters_outside_ascii(value: str) -> None:
...
@pytest.mark.parametrize("value", ["seo_yeon", "seoyeon!", "서연1234"])
def test_require_login_id_rejects_anything_but_lowercase_letters_and_digits(value: str) -> None:
    with pytest.raises(ValueError, match="영어 소문자와 숫자"):
        require_login_id(value)


def test_require_login_id_accepts_lowercase_letters_and_digits() -> None:
    assert require_login_id(" SeoYeon1 ") == "seoyeon1"


# 서비스가 다루지 못하는 기호를 비밀번호에 넣지 못하게 합니다. 목록에 없는 기호는 거부합니다.
@pytest.mark.parametrize("value", ["Abcdef1<", "Abcdef1'", "Abcdef1 x", 'Abcdef1"'])
def test_require_password_rejects_symbols_outside_the_list(value: str) -> None:
    with pytest.raises(ValueError, match="특수문자는"):
        require_password(value)
