 ...
 *  disabled 인 선택지는 목록에 표시하되 선택할 수 없습니다(이미 찬 시각 등). */
export type Choice<T> = { value: T; label: string; disabled?: boolean };

/** from 에서 step 칸만큼 이동한 뒤, 선택할 수 없는 선택지를 건너뛴 위치를 반환합니다.
 *  step 은 아래로 이동할 때 1, 위로 이동할 때 -1 입니다. 양 끝에서는 더 이동하지 않고 현재 위치를 유지합니다.
 *  현재 위치도 선택할 수 없으면 -1 을 반환합니다. */
...
/** 목록의 처음(step 이 1) 또는 끝(step 이 -1)에서 시작해 처음으로 선택할 수 있는 선택지의 위치입니다.
 *  Home·End 키가 사용합니다. 선택할 수 있는 선택지가 없으면 -1 입니다. */
export function edgeChoice<T>(choices: Choice<T>[], step: number): number {
  return moveChoice(choices, step > 0 ? -1 : choices.length, step);
}

/** 목록을 열 때 커서를 둘 위치입니다. 지금 선택한 값을 선택할 수 있으면 그 자리, 아니면 첫 번째입니다. */
