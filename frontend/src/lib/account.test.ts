...
describe("can", () => {
  ...
  it("permissions 항목이 아예 없는 응답도 없는 것으로 본다", () => {
    // 낡은 서버가 permissions 없이 답하는 경우입니다. 타입에는 있지만 값이 없을 수 있습니다.
...
describe("PERMISSION_ITEMS", () => {
  it("서버가 고정한 열여덟 가지를 그 순서대로 든다", () => {
    // 만들기·수정·삭제·주기를 따로 둡니다. 서버 쪽 정본은 backend/db/models.py 의
    // Permission 이고, 이 목록이 그것과 개수가 어긋나면 화면에 못 켜는 항목이 생깁니다.
