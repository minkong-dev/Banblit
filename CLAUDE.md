...
**`dev_history/` 의 가장 최근 파일 하나를 읽는다.** 현재 상태, 지금 하던 일, 남은 것이
전부 거기 있다. 그 파일이 가리키기 전에는 저장소를 훑지 않는다.

이 규칙은 세션을 짧게 유지하기 위한 것이다. 컨텍스트는 요청마다 통째로 다시 나가므로
세션이 길어질수록 같은 질문 하나가 몇 배씩 비싸진다. 파악을 파일에서 하면 `/clear` 가
공짜가 되고, 작업 하나가 끝날 때마다 끊을 수 있다.

---
## 1. 이 프로젝트의 에이전트

`.claude/agents/` 에 있는 열한 개가 전부다. ECC(`affaan-m/ECC`)에서 필요한 것만 옮겨 왔고,
플러그인 자체는 꺼 두었다 — 68개를 다 켜면 요청마다 40,637 토큰이 붙는데 이 열한 개의
몫은 671 토큰이다.
...
서브에이전트는 **중간 산출물을 확인하지 않아도 되는 일**에만 띄운다.
훑을 양이 6만 자를 넘고 돌려받을 것이 한 문단일 때가 그 경우다.
고칠 파일과 내용이 정해진 편집은 직접 한다.
여러 개를 동시에 띄우는 것은 시간이 급할 때만 한다. 비용은 개수만큼 늘어난다.

서브에이전트 하나의 고정비는 프리앰블 38,000 토큰이고, 그 값은 그 에이전트가 도는 내내
매 요청마다 다시 나간다. 유일한 이득은 그 에이전트가 읽은 것이 끝나면 버려진다는 것뿐이다.
결과를 이쪽에서 검토해야 하는 일은 내용이 어차피 이 대화로 들어오므로 이득이 없다.

위임할 때는 파일 경로와 줄 번호, 무엇을·왜·어떻게를 빠짐없이 넘긴다 — 서브에이전트는
이 대화를 보지 못한다.
목록에 없는 에이전트가 필요해지면 ECC 캐시에서 그 파일 하나만 `.claude/agents/` 로 옮기고
표에 적는다. 플러그인을 통째로 켜지 않는다.

---
## 2. 이 프로젝트의 스킬

`.claude/skills/` 에 있는 것이 전부다. 아래 스무 개는 ECC에서 옮겨 왔다.
...
ECC 밖의 스킬은 네 가지를 쓴다 — 문서는 `cluedoc`(`KeunwooPark/cluedoc`, `.claude/skills/` 에 있다),
외부 포맷 내보내기는 `document-skills`, Claude API 연동은 `claude-api`, 그리고 `ui-ux-pro-max`.

ECC의 언어별 규칙 문서는 `.claude/rules/{common,python,react,typescript}/` 에 있다.
스킬과 에이전트가 이 경로를 참조한다. 주석·가독성 규칙이 `refactoring` 스킬과 어긋나면
`refactoring` 이 정본이다.
...
구현은 공통 규칙 4장의 **모드 3(TDD 후 ponytail)** 로 한다. 실패하는 테스트를 먼저 쓰고,
실패하는 모습을 직접 본 다음에만 구현으로 넘어간다.

**리뷰는 기능 하나가 끝났을 때 한 번 돌린다.** 덩어리마다가 아니다.
대상은 저장소가 아니라 `git diff` 로 한정한다 — 검토자가 읽어야 할 양이 두 자릿수 배로 줄고,
바뀌지 않은 코드를 다시 훑는 일이 없어진다.

해당 언어 검토자 하나를 고른다(`python-reviewer` `fastapi-reviewer` `database-reviewer`
`react-reviewer`). 보안이 걸린 diff 일 때만 `security-reviewer` 를 더한다.
`code-reviewer` 는 언어 검토자가 없는 diff 에 쓴다. 셋을 한꺼번에 돌리지 않는다.
...
**이 파일은 0장이 정한 세션 진입점이기도 하다.** 다음 세션이 이것만 읽고 바로 이어서
일할 수 있어야 한다. 그러려면 맨 앞에 **"지금 상태"** 를 둔다 — 지금 무엇을 하던 중이고,
