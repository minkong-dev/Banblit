...
```
docker compose logs api --since 48h | grep "메일"
```

- **실행 경로**: 저장소 루트 (`Banblit/`)
- **용도**: 비밀번호 재설정·아이디 안내 메일을 보냈는지, 보내지 않았다면 사유가 무엇인지 확인합니다.
  "메일을 못 받았다"는 문의가 들어왔을 때 서버가 보내지 않은 것인지 수신 측 문제인지 구분하는 근거입니다.
- **출력되는 문구 4가지**
  - `비밀번호 재설정 메일을 발송했습니다 — 계정 12` — 발송까지 정상입니다. 못 받았다면 스팸함을 확인합니다.
  - `… 사유: 일치하는 계정 없음` — 그 이메일로 가입한 계정이 없습니다.
  - `… 사유: 재발송 제한 1분 이내, 계정 12` — 1분 안에 다시 요청했습니다(`RESEND_INTERVAL`).
  - `아이디 안내 메일을 발송하지 않았습니다 — 사유: 이름 불일치, 계정 12` — 이메일은 일치하고 이름이 다릅니다.
- **옵션**
  - `--since 48h` — 최근 48시간치만 출력합니다. 생략하면 이전 기동의 로그까지 함께 출력합니다.
  - `grep "메일"` — 메일 관련 기록만 남깁니다. 발송 실패 기록(`메일을 보내지 못했습니다`)도 같은 낱말을 포함해 함께 출력됩니다.
- **주의**
  - 기록에는 계정 번호(`member.id`)만 남습니다. 누구인지 확인하려면 `4-4`(DB 접속)로 `select id, name, email from members where id = 12;` 를 조회합니다.
  - `docker compose logs` 로 확인하려면 API 서버가 `backend.api.app` 으로 기동되어 있어야 합니다. 기록 출력 설정을 이 파일이 담당합니다.
