...
- **실행 경로**: 저장소 루트 (`Banblit/`)
- **용도**: 화면 쪽 검사를 돌립니다. 지금 덮는 것은 순수 계산과 입력 검증이다 — 서버 호출 감싸개, 달력 칸 계산, slot 조각을 합주로 잇는 계산, 계정 서식 입력 검사, 삭제 확인 문구의 조사 붙이기. 통과하면 `Tests 229 passed` 가 나옵니다. 화면을 실제로 띄워 보는 검사는 아직 없습니다.
- **옵션**
