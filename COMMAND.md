...
- **주의점**
  ...
  - `tests/unit` 의 검사가 실제 DB 픽스처(`test_engine`·`db_session`·`api_client`)를 쓰면 수집 단계에서 멈춥니다. DB 가 떠 있는 동안 오류 메시지 없이 통과해 버리는 것을 막기 위해서입니다.
...
- **주의점**
  ...
  - 달력에 데이터가 보이려면 `4-2` 마이그레이션이 들어가 있고 팀·기간·배정이 실제로 등록돼 있어야 합니다. 없으면 화면은 정상적으로 뜨고 "저장된 배정이 없습니다" 고 말합니다.
...
- **실행 경로**: 저장소 루트 (`Banblit/`)
- **용도**: 화면 쪽 검사를 돌립니다. 지금 덮는 것은 순수 계산과 입력 검증입니다 — 서버 호출 감싸개, 달력 칸 계산, slot 조각을 합주로 잇는 계산, 계정 서식 입력 검사, 삭제 확인 문구의 조사 붙이기. 통과하면 `Tests 229 passed` 가 나옵니다. 화면을 실제로 띄워 보는 검사는 아직 없습니다.
- **옵션**
...
- **실행 경로**: 저장소 루트 (`Banblit/`)
- **용도**: 지금 고치는 파일의 검사만 골라 돌립니다. 실패하는 검사를 먼저 써 놓고 실패하는 모습을 볼 때, 전체를 다 돌리지 않아도 됩니다.
- **옵션**
  - `run --rm --no-deps web` — `10-2` 와 같습니다.
  - `npx vitest run <파일...>` — `npm test` 대신 vitest 를 직접 부릅니다. `run` 은 한 번 돌고 끝나는 것이고, 뒤에 적은 경로는 `frontend/` 기준입니다. 여러 개를 띄어 적으면 그것들만 돕니다. 경로는 부분 일치라 `validate` 만 적어도 `validate.test.ts` 가 잡힙니다.
- **주의점**
  - 고친 것이 다른 파일의 검사를 깨뜨릴 수 있으니, 구현이 끝나면 `10-2` 로 전체를 한 번 돌립니다.
...
- **주의점**
  - **`frontend/eslint.config.js` 는 이 저장소의 `config-protection` 훅이 에이전트의
    ...
    선호가 아니라 이 제약 때문입니다 — 한 파일로 합치려면 사람이 직접
