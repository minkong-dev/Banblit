...
- **주의점**
  ...
  - 같은 이메일로 1행을 두 번 실행하면 `422`("이미 가입된 이메일입니다")가 돌아온다. 다시 확인할 때는 이메일을 바꾼다.
...
- **주의점**
  ...
  - 달력에 데이터가 보이려면 `4-2` 마이그레이션이 들어가 있고 팀·기간·배정이 실제로 등록돼 있어야 한다. 없으면 화면은 정상적으로 뜨고 "저장된 배정이 없다" 고 말한다.
...
- **주의점**
  ...
  - 이 확인은 화면과 넘기기만 본다. 데이터가 보이려면 `4-2` 마이그레이션이 들어가 있고 팀·기간·배정이 실제로 등록돼 있어야 한다.
...
`banblit.ps1` 하나가 아래 13개 절의 명령을 순서대로 묶어 실행한다. 개별 `docker compose`
...
- **실행 경로**: 저장소 루트 (`Banblit/`)
- **용도**: PowerShell profile 에 `banblit` function 을 넣는다. 등록하면 다른 폴더에서도
  `banblit up` 으로 쓸 수 있다. 등록하지 않아도 저장소 루트에서 `.\banblit.ps1 up` 으로 쓸 수 있다.
- **옵션**
  - `-Remove` — 등록한 function 을 profile 에서 지운다. 생략하면 등록한다.
- **주의점**
  - alias 가 아니라 function 으로 넣는다. alias 는 뒤따르는 인자를 넘기지 못하는 경우가 있다.
  - `# >>> banblit >>>` 와 `# <<< banblit <<<` 사이에만 쓰므로 여러 번 실행해도 쌓이지 않는다.
  - 대상 profile 은 `$PROFILE.CurrentUserAllHosts` 다. 지금 열려 있는 창에도 바로 반영한다.
...
- **용도**: 아래를 순서대로 실행한다.
  1. `banblit-backend:dev` 이미지가 `backend/pyproject.toml`·`uv.lock`·`Dockerfile` 보다
     오래되었으면 `1-1`(`docker compose build dev`)을 먼저 실행한다.
  2. `4-2`(`alembic upgrade head`)로 마이그레이션을 맞춘다.
  3. `3-1`·`10-1`로 `api` 와 `web` 을 띄운다.
  4. `/health` 와 5173 에 실제로 요청해 응답할 때까지 기다린다.
  5. 가입된 계정이 있는지 세어 알려 준다.
  6. 브라우저로 화면을 연다.
- **옵션**
  - `-Auto` — `13-1`의 자동 배정 서비스까지 함께 띄운다. 생략하면 띄우지 않는다.
    개발용 override 가 기본을 꺼짐으로 두기 때문에 이 switch 로만 켜진다.
  - `-Build` — 이미지 나이와 무관하게 무조건 다시 만든다. 생략하면 나이를 보고 판단한다.
  - `-NoBrowser` — 브라우저를 열지 않는다. 생략하면 연다.
- **주의점**
  - 컨테이너가 `Up` 이어도 uvicorn 이 import 단계에서 죽어 있을 수 있으므로, `docker compose ps`
    가 아니라 실제 응답으로 확인한다. API 는 120초, 화면은 420초까지 기다린다.
  - 화면은 처음 띄울 때 컨테이너 안에서 `npm install` 이 돌아 몇 분 걸린다.
  - 의존성을 추가하고 이미지를 다시 만들지 않으면 API 가 import 단계에서 죽는다.
    1번 단계가 그것을 막는다.
...
- **실행 경로**: 어디서나
- **용도**: `1-2`(`pytest -q`)와 `1-2-3`(`mypy`)을 차례로 돌린다. 화면에는 통과 여부와
  실패한 항목만 내고, **전체 출력은 `.logs/pytest-<시각>.log`·`.logs/mypy-<시각>.log` 에 남긴다.**
- **옵션**: 없다.
- **주의점**
  - 화면 출력을 줄이는 것이 목적이다. 전체 출력이 필요하면 `.logs/` 의 파일을 보거나
    `1-2`·`1-2-3`을 직접 실행한다.
  - `.logs/` 는 `.gitignore` 에 있다. 커밋되지 않는다.
  - 하나라도 실패하면 종료 코드 1 로 끝난다.
...
- **용도**: `docker compose ps` 로 컨테이너 목록을 보이고, 8000 과 5173 에 실제로 요청해
  응답 여부를 확인한다.
- **옵션**: 없다.
- **주의점**: 컨테이너가 `Up` 인 것과 앱이 응답하는 것은 다르다. 이 명령은 둘 다 본다.
...
- **실행 경로**: 어디서나
- **용도**: `3-3`으로 서비스를 내린다. 데이터는 volume 에 남는다.
- **옵션**
  - `-Volumes` — volume 까지 지운다. DB·첨부파일·테스트용 DB 가 전부 사라진다.
    `yes` 를 직접 입력해야 실행된다. 생략하면 volume 을 남긴다.
...
- **실행 경로**: 어디서나
- **용도**
  - `logs` — `3-1-2`. 최근 1분 기록을 본다. 대상을 적지 않으면 전부.
  - `restart` — 다시 띄운다. 대상을 적지 않으면 `api` 와 `web`.
  - `migrate` — `4-2`·`4-2-1`. 마이그레이션만 맞추고 현재 revision 을 낸다.
  - `help` — 명령·service·switch 목록.
- **옵션**
  - 첫 번째 위치 인자가 명령, 두 번째가 대상 service 다. service 는
    `api` `web` `db` `auto-assign` 넷 중 하나이고, `logs` 와 `restart` 에서만 쓴다.
  - `-Follow` — `logs` 를 붙잡고 계속 본다. 생략하면 한 번 내고 끝난다.
- **주의점**
  - 명령을 적지 않으면 `up` 이 기본값이다.
