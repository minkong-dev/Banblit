...
**저장 버튼을 누를 때 한 번에 전송합니다.** 팀(POST 또는 PATCH·PUT slots)을 먼저 저장하고, 서버 자리 목록을 다시 조회해 포지션·번호로 대응시킨 뒤 멤버가 변경된 자리만 요청합니다(`lib/roster.ts` 의 `seatChanges`). **변경된 자리 중 멤버가 있던 자리를 전부 먼저 해제하고 그 다음 지정합니다.** 서버가 한 멤버를 한 팀의 두 자리에 배치하는 요청을 거절하므로(`team_slots_team_id_member_id_key`), 두 멤버의 자리를 교환할 때 해제하지 않고 지정하면 거절됩니다.
