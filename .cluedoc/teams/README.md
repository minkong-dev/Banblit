...
**어떻게 확인했나.** 팀·포지션 endpoint를 확인하는 테스트가 로그인하지 않은 요청과 권한이 없는 요청을 서로 다른 응답으로 가르는지 봅니다. 구성을 고칠 때 앉아 있던 사람이 남는지, 줄일 때 번호가 큰 쪽부터 없어지는지, 한 사람이 같은 팀의 두 포지션을 맡으려 하면 거절되는지, 팀을 지우면 포지션이 함께 지워지는지도 같은 파일이 봅니다. 서버 테스트 501개와 화면 테스트 200개가 모두 통과합니다.
