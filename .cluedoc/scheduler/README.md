...
| 상태 | 카드가 보이는 모습 | 누르면 |
| --- | --- | --- |
| 안 읽은 알림이 있습니다 | 머리글에 개수가 표시되고, 줄마다 안 읽음 표시가 붙습니다 | 전부 삭제되고 목록이 "새 알림이 없어요" 로 바뀝니다 |
| 전부 읽었습니다 | 개수도 표시도 없습니다 | 아무 일도 일어나지 않습니다 |
| 아직 조회하지 못했거나 조회에 실패했습니다 | 조회 중 또는 실패 안내를 1줄로 표시합니다 | — |

머리글을 눌러도 줄 1개를 눌러도 전체가 삭제됩니다. 알림을 1개씩 읽는 화면은 미구현이므로 어느 쪽을 눌러도 결과가 같습니다. 목록에 남아 있는 알림은 전부 읽지 않은 알림이라, 화면은 읽은 줄과 안 읽은 줄을 구분하지 않습니다.

색으로만 알리지는 않습니다. 줄에 붙는 점에는 "안 읽음" 텍스트가 있어 스크린 리더가 "안 읽음"으로 읽습니다. 색만으로 표시하면 스크린 리더를 쓰는 사용자에게는 그 표시가 전달되지 않습니다.
...
집중 합주기간        →  누를 수 없다 (전체합주 날짜는 예외)
```

"목요일 7시에 두 시간 쓰고 싶은데 언제 비어 있나"가 사용자의 실제 질문이기 때문입니다. 날짜를 1개씩 열어 확인하는 대신 시간을 먼저 선택하고 예약 가능한 날짜만 남깁니다.

집중 합주기간은 예약을 전부 차단합니다. 자동 배정이 모든 팀에 slot을 배정하고도 남는 slot이 생기지만, 그 slot도 예약 대상이 아닙니다. 예외는 전체합주 날짜입니다 — 그날은 팀별 배정이 없으므로 예약을 허용하고, 전체합주에 지정한 합주실의 전체합주 시각과 겹치는 구간만 거절합니다. 남는 slot이 생기는 이유는 [합주실과 기간](../practice-room-and-periods/README.md)에서 다룹니다.
...
| 오른쪽 | 내 일정 목록 | 내 일정 탭은 모달을 연 날짜와 무관하게 **내가 등록한 불가능 일정 전부**를 시작 시각 순으로 나열합니다(`lib/dayEntries.ts` 의 `allOffEntries`). 줄마다 날짜와 시각을 적고, 반복 일정은 전개하지 않고 1줄로 나열하며 날짜 뒤에 "· 매주"·"· 매일" 을 적습니다. 예약 탭은 그 날짜에 내가 한 예약만 나열합니다. 두 목록 모두 줄마다 삭제 아이콘이 있습니다 |
...
처음에는 계산이 1시간 칸을 1개씩 배정해 서버가 반환하는 시간표가 칸의 나열이었고, 화면이 같은 팀이 같은 합주실에서 연속으로 사용하는 칸을 합쳐 "합주 1회"로 표시했습니다. 2026-09-20 부터 계산이 합주 1회를 이어진 구간으로 배정하고 저장도 구간 한 행이라, 서버가 반환하는 행이 이미 합주 1회입니다. 화면이 세는 단위는 처음부터 합주 1회였으므로 표시 방식은 그대로입니다.
...
달력은 집중 합주기간 전부를 표시합니다. 달 보기의 날짜 칸은 어느 집중 합주기간의 팀별 배정 날짜에든 속하면 집중 합주일로 표시하고, 달력 아래 띠는 범위마다 "<시작일> ~ <종료일>" 을 쉼표로 이어 표시합니다. "매일" 기간은 종료일 자리에 "매일" 을 표시합니다. 서버가 집중 합주기간끼리의 날짜 겹침을 거절하므로 한 날짜는 기간 하나에만 속합니다(`lib/calendar.ts` 의 `focusedRanges`·`inRanges`, 테스트는 `calendar.test.ts`).

전체합주 날짜는 팀별 배정 날짜에서 제외되므로, 전체합주가 기간 중간에 있으면 띠의 범위가 앞뒤 두 개로 나뉘고 그 날짜는 예약 탭에서 "자동 배정" 으로 막히지 않습니다. 대신 그날 전체합주가 달력 항목 하나(`kind: "ensemble"`, `lib/dayEntries.ts` 의 `ensembleByDay`)로 들어가, 예약 탭은 지정 합주실의 그 시간을 찬 칸으로 계산하고 내 일정·전체 일정 탭은 모든 멤버에게 표시합니다. 막대는 팀 색 대신 세이지색 빗금(`.ens`)이며 범례에 "전체합주" 가 붙습니다. "스케줄링 시간 설정" 권한이 있으면 하루 dialog 에 "이날 전체합주 시각" 입력이 표시되어 그 날짜의 시각을 저장하거나 기본 시각으로 되돌립니다(`routes/SettingsEnsemble.tsx` 의 `EnsembleDayEditor`). 날짜별 시각 우선 규칙은 `dayEntries.test.ts` 의 `ensembleOn` 이 확인합니다. 화면 전체의 미구현 항목은 [화면](../screens/README.md)이 표로 정리합니다.
