...
사람에 붙는 두 테이블(권한 묶음, 사람↔묶음)과 기간에 붙는 한 테이블(자동 실행 기록)이 이번에 늘었습니다. 무엇을 켜고 끄는지는 [계정과 역할](../accounts-and-roles/README.md)이, 언제 무엇이 돌았다고 남기는지는 [자동 배정](../auto-assignment/README.md)이 정본이고, 여기서는 그 규칙이 저장소에서 어떻게 지켜지는지만 다룹니다.

참가 승인은 그 뒤에 통째로 걷어냈습니다. 소속 표가 사라지고 **팀 포지션** 표가 들어왔는데, 팀이 악기별로 몇 자리를 갖는지가 팀의 정의가 되고 사람은 그 자리를 채웁니다. 무엇이 왜 바뀌었는지는 [팀과 포지션](../teams/README.md)이 정본이고, 여기서는 저장소가 그 구조를 어떻게 지키는지만 다룹니다.
...
| 팀은 악기별 포지션 구성으로 정의된다([팀과 포지션](../teams/README.md)이 정본) | 팀 포지션 표가 (팀 + 악기 + 몇 번째)를 들고, 그 조합에 중복 금지 조건을 건다. 악기는 정해진 여섯 가지 밖으로 나가면 거부하고, 몇 번째인지는 1 이상이어야 한다. 앉은 사람 칸은 비어 있을 수 있다 — 사람 없는 포지션이 정상 상태다. 한 사람이 같은 팀의 두 포지션을 맡지 못하도록 (팀 + 사람)에도 중복 금지를 건다 |
...
| 합주실의 여닫는 시각은 정시 위에만 있어야 한다(사용자 결정 — 칸이 1시간이 되면서 좁혔다) | 합주실 테이블에 여는 시각·닫는 시각의 분이 0인지 검사하는 조건을 건다 |
| 합주실은 닫는 시각이 여는 시각보다 뒤여야 한다(뒤집힌 영업시간 금지) | 합주실 테이블에 "닫는 시각 > 여는 시각" 조건을 건다 |
| 기간은 두 종류뿐이고, 종류와 무관하게 하루 2회 연산 시각은 반드시 지정한다 | 기간 테이블이 종류를 두 값 중 하나로만 제한하고, 두 연산 시각 열을 반드시 채우도록 강제한다. 예약할 수 있는 날인지는 저장된 기간이 아니라 서버가 판정한다 — 집중 합주기간이 덮지 않은 날이면 예약제다 |
| 불가능시간은 반복 등록(매주 반복 + 반복 종료일)을 지원하며, 끝나는 시각이 시작 시각보다 앞설 수 없다 | 불가능시간 테이블이 매주 반복 여부와 반복 종료일을 갖고, 시작·끝 시각이 뒤집힌 구간은 저장을 거부한다 |
| 모든 점유는 1시간 slot 단위이고, slot 하나에는 배정이 하나만 들어간다(선착순 1개, 이중 예약 금지) | 배정 테이블이 "합주실 + 시작 시각" 조합에 중복 금지 조건을 걸어, 같은 slot에 두 팀이 동시에 들어가는 저장 자체를 막는다. 점유가 항상 정시에서만 시작한다는 규칙이 지켜지는 한, 시작 시각이 다른 두 배정끼리 시간이 겹치는 일은 애초에 생기지 않는다 — 별도의 구간 겹침 방지 장치는 두지 않는다. 예약도 같은 slot 구조 위에 얹는다 |
| 예약도 배정과 같은 1시간 slot 구조 위에 얹는다 | 예약 테이블이 배정 테이블과 같은 얼개다 — "합주실 + 시작 시각" 조합에 중복 금지 조건을 걸어 한 slot에 하나만 들어가게 하고, 끝나는 시각이 시작 시각보다 앞선 예약은 거부한다. 여러 slot을 이어 쓴 예약은 slot 수만큼 행으로 남는다. 팀을 적은 자리는 비워둘 수 있으며, 비어 있으면 그 예약은 개인이 직접 잡은 것이다 |
...
| 권한은 두 값짜리 역할이 아니라 정해진 항목 열여덟 가지를 켜고 끄는 묶음으로 정한다([계정과 역할](../accounts-and-roles/README.md)이 정본) | 사람 테이블에 있던 역할 열을 없앴다. 대신 권한 묶음 테이블이 이름 하나와 설명 한 줄과 켜진 항목 목록을 담고, 이름에 중복 금지 조건을, 항목 목록에는 "정해진 열여덟 가지 밖의 이름이 섞이면 거부"라는 조건을 건다. 설명은 비워 둘 수 없다. 항목 목록 자체는 데이터가 아니라 코드가 고정한다 — 항목이 늘면 그것을 실제로 막는 서버 코드도 같이 늘어야 하므로, 데이터로만 늘릴 수 있게 두면 켤 수는 있는데 아무것도 막지 못하는 항목이 생긴다 |
...
| 같은 사람이 두 번 들어오지 않는다 — 이름·학과·학번·기수 네 가지가 모두 같으면 같은 사람이다(사용자 결정) | 사람 테이블의 그 네 열 조합에 중복 금지 조건을 건다. 이 조건이 생기기 전에 들어온 행은 학과·학번이 비어 있는데, 저장소가 빈 값이 낀 조합을 겹침으로 보지 않아 옛 행끼리는 부딪히지 않는다 |
| 사람을 지우면 그 사람이 남긴 글·댓글·예약도 함께 사라진다(사용자 결정) | 글·댓글·예약이 사람을 가리키는 연결을 "함께 지움"으로 바꿨다. 그 전에는 "붙들기"였고, 붙들고 있으니 탈퇴 자체가 막혔다. 팀 포지션만 예외로 "사람만 비움"이다 — 포지션은 팀의 구성이라 사람이 나갔다고 사라지면 안 된다 |
...
| 예약 → 합주실 / 팀 | 참조 대상과 함께 삭제 | 합주실이 없어지면 그 방의 slot을 잡아둔 예약도 성립하지 않고, 팀이 없어지면 그 팀 이름으로 잡힌 예약도 마찬가지다 |
...
위 파일들을 포함한 서버 테스트 484개가 통과하고, 타입 검사는 104개 파일에서 이상이 없으며, 화면 테스트 170개가 통과함을 확인했습니다.
...
두 열을 더하는 마이그레이션은 데이터를 넣어둔 채로 올렸다 내렸다 왕복해 확인했습니다. 되돌리면 대기 중인 신청은 사라지고 승인된 소속만 남는데, 되돌리는 쪽이 대기 중인 행을 먼저 지우기 때문입니다. 그러지 않으면 상태 열이 없어지는 순간 신청이던 행이 전부 소속으로 둔갑합니다. 이 층만 따로 확인하려면:
