...
이번에 들어온 세 가지 — 게시판 파일 첨부, 화면 안 알림, 메일로 하는 비밀번호 재설정·아이디 찾기 — 는 서버 테스트 468개와 화면 테스트 202개가 통과하고 타입 검사가 94개 파일에서 이상 없음을 확인했습니다. 첨부는 여기에 더해 실제로 파일을 올리고 다시 받아 한 바이트도 다르지 않은 것, 허용하지 않은 종류가 거절되는 것, 게시글을 지우면 파일이 사라지는 것을 눈으로 확인했습니다.
