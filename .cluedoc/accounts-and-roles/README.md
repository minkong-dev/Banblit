...
**이름이 같아도 가입된다.** 동아리에는 동명이인이 있다. 가입하면 번호가 하나 붙고, 이후 모든 화면은 이름과 번호를 함께 보여준다. 사람을 가르는 기준은 이름이 아니라 번호다. 이 사실은 가입 화면에서 미리 알린다.

입력값은 보내기 전에 화면에서 한 번 거른다 — 빈칸, 이메일 형식, 비밀번호 길이, 두 비밀번호가 다른 경우, 포지션을 하나도 안 고른 경우. 문제가 있는 칸 바로 아래에 무엇이 잘못됐는지 적는다.
