...
sources:
  ...
  - backend/src/backend/scheduling/slots.py          # 운영시간 → 30분 slot
...
- [합주실과 기간](../practice-room-and-periods/README.md) — 합주실별 운영 시간과 30분 slot 규칙
...
slot은 합주실별로 만들어집니다. 각 합주실이 여는 시간을 30분 slot으로 쪼개고 그 slot 하나가 배정의 최소 단위이며, 방마다 여는 시간이 달라도 slot의 경계는 모든 방이 공유합니다.
...
결과에는 팀별로 배정된 slot 목록이 담기며 각 slot은 어느 합주실의 어느 시간인지를 함께 담습니다. 여기에 팀들이 slot을 받고 남은 예약 가능한 자리 목록이 따라붙는데, 상시 예약으로 쓸 수 있는 자리로 화면에 보입니다.
...
| 운영 시간이 30분 격자를 벗어남 | 존재하지 않는 slot이 만들어지거나 자투리가 조용히 사라짐 |
...
네 자리를 서로 다른 항목으로 나눈 것이 실제로 쓸모가 있습니다. 계산은 못 돌리지만 남이 돌린 결과를 검토만 하는 사람, 되돌리기만 맡은 사람을 따로 둘 수 있습니다. 열한 개를 모두 켠 사람에게는 예전과 똑같이 동작하므로 옮겨오면서 할 수 있는 일이 줄어든 사람은 없습니다.
