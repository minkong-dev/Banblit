...
확인은 저장에 성공했을 때만 저장되는지, slot을 받은 팀의 사람에게만 저장되는지, 팀이 없는 사람과 승인 대기 중인 사람은 제외되는지, 2개 이상 팀에 속한 사람에게 1행만 저장되는지, 읽음 표시가 읽지 않은 행에만 저장되는지를 테스트에 담아서 했습니다. 그 테스트에 사람이 버튼을 눌러 다시 계산해도 알림이 저장되는지와 되돌리기에도 알림이 저장되는지 2가지를 추가했습니다. 테스트 파일은 맨 위 목록의 알림 시나리오 파일 1개이고, 서버 테스트 전체 468개가 통과하며 타입 검사는 94개 파일에서 이상이 없습니다.
