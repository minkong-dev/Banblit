...
검증은 여섯 자리 모두 로그인하지 않은 요청을 거절하는지, 그중 항목을 요구하는 네 자리는 그 항목이 없는 사람의 요청을 그와 다른 답으로 거절하고 항목을 가진 사람만 통과시키는지를 확인하는 테스트로 했습니다. 서버 테스트 전체 468개가 통과하고 타입 검사는 94개 파일에서 이상이 없으며, 실행 커맨드는 [스케줄링 API](../scheduling-api/README.md)에 적어 뒀습니다.
