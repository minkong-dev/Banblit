...
flowchart TD
    ...
    V -->|아니오| E[error로 거절<br/>무엇이 잘못되었는지 지목]
    V -->|예| S[모든 조건을 1회 계산으로 충족<br/>전원 가능 · 합주실별 session · 사람 중복 방지 · 같은 횟수]
    S --> Q{조건을 충족하는<br/>배정을 찾았는가}
    Q -->|찾음| OK[성공 → 즉시 확정<br/>남은 칸은 빈 자리로 반환]
...
자동 배정은 집중 합주기간에만 실행되는 계산입니다. 멤버들의 불가능 시간을 모아 각 팀이 전원 참석할 수 있는 시간에 합주 1회를 배치합니다. 합주 1회는 설정한 길이만큼 끊기지 않고 이어지는 구간이고, 시작할 수 있는 시각은 시간 칸의 경계입니다. 기간마다 정해둔 고정 시각에 하루 2회 자동으로 실행되고, 사람이 버튼을 눌러 실행할 수도 있습니다. 계산은 모든 조건을 동시에 충족하는 배정을 1회 계산으로 찾는 방식이고, 그런 배정이 없으면 "누구를 제외하면 배정이 가능한지"를 제안으로 출력합니다. 배정 결과에는 어느 합주실의 어느 시간인지가 함께 담기며, 어떤 팀도 차지하지 않은 칸은 빈 자리로 함께 나옵니다. 빈 자리도 집중 합주기간 안이라 예약할 수 없습니다.
...
- [합주실과 기간](../practice-room-and-periods/README.md) — 합주실별 운영 시간, 칸 크기와 합주 1회 길이 규칙
...
단위는 2개입니다. 시간 칸은 합주를 **시작할 수 있는 간격**이고, session(합주 1회가 이어지는 구간)은 **한 번 시작하면 몇 분 동안 이어지는지**입니다. 두 값 모두 저장소 설정이 정하고, session 길이는 칸 크기의 배수여야 합니다. 30분 칸에 60분 session 이면 18:00·18:30·19:00 중 어디서 시작하든 1시간 뒤에 끝납니다.

시간 칸은 합주실별로 생성됩니다. 각 합주실이 여는 시간을 칸 크기로 분할하고, 합주실마다 여는 시간이 달라도 칸의 경계는 모든 합주실이 공유합니다.

배정의 단위는 칸이 아니라 session 입니다. 이전에는 칸 1개가 배정의 단위여서, 30분 칸으로 설정하면 한 팀이 18:00·20:30·22:00 처럼 떨어진 30분 칸을 따로따로 받는 것이 정상 동작이었습니다. 실제 합주는 한 번 시작하면 그 길이만큼 이어지므로, 계산은 이어진 구간 1개를 1회로 배치합니다.

```
칸 크기 30분 · session 60분 · 1번 합주실이 18~20시 개방
...
후보끼리 겹치므로, 하나를 배정하면 그 구간의 칸 2개가 모두 찹니다.
```

### 대상은 이름이 아니라 번호로 구분합니다

계산은 합주실·팀·사람을 모두 번호로만 구분하고 이름은 계산에 들어가지 않습니다. 사람은 동명이인이 있어 이름으로는 구분되지 않고, 합주실은 같은 합주실이 날짜마다 다시 쓰이므로 이름 1개로는 날짜를 구별할 수 없기 때문입니다. 칸 1개는 합주실 번호와 시작 시각 2개 값이 함께 가리키고, 그 2개 값이 같은 칸이 2회 나오면 계산 전에 거절합니다.
...
│ 합주실 번호   │ 시작 시각               │
├──────────────┼────────────────────────┤
│      7       │ 2026-08-01 18:00       │  ← 서로 다른 칸
│      7       │ 2026-08-02 18:00       │  ← 같은 합주실, 다른 날
│      9       │ 2026-08-01 18:00       │  ← 같은 시각, 다른 합주실
└──────────────┴────────────────────────┘
```
...
| 조건 | 뜻 |
| --- | --- |
| 전원 가능 | 팀은 그 팀 멤버 전원이 session 전체 구간에 가능한 시간에만 배치 |
| 칸 1개에 팀 1개 | 같은 합주실의 같은 칸을 2개의 session 이 함께 차지할 수 없음 |
| 팀 1개는 합주실 1곳에 | 팀 1개가 같은 시간에 합주실 2곳을 동시에 쓸 수 없음 |
| 사람 중복 방지 | 2개 이상 팀에 속한 사람이 같은 시간에 2곳에 있을 수 없음 |
| 같은 횟수 | 모든 팀이 정확히 같은 횟수의 session 을 가짐 |

두 번째 조건이 칸 단위인 이유는 session 후보끼리 겹치기 때문입니다. 18:00 시작과 18:30 시작은 서로 다른 후보지만 18:30~19:00 칸을 함께 차지하므로, 후보 2개가 아니라 그 후보가 차지하는 칸을 세어 충돌을 판정합니다.

마지막 조건은 "공정하게 잘 나눈다"는 계산이 아니라 "모두 같은 횟수"라는 단순한 규칙이고, 횟수를 채우지 못하는 팀이 1개라도 있으면 배정 전체가 실패로 처리됩니다. 팀마다 가질 횟수는 겹치지 않게 들어가는 session 총 횟수를 팀 수로 나눈 몫입니다.

결과에는 팀별로 배정된 session 목록이 담기며 각 session 은 어느 합주실의 어느 구간인지를 함께 담습니다. 이 결과에 어떤 session 도 차지하지 않은 칸 목록(`open_slots`)이 추가됩니다. 이 목록은 session 단위가 아니라 칸 단위입니다 — 남은 시간이 session 1회보다 짧아도 선착순 예약은 그 칸을 쓸 수 있기 때문입니다. 이 목록은 어느 자리가 비었는지를 알려줄 뿐 예약 대상이 아닙니다 — 집중 합주기간의 예약은 서버가 전부 거절합니다. 지금은 화면이 이 목록을 사용하지 않습니다.

배정을 찾으면 성공으로 판정하고 즉시 확정합니다. "팀 간 시간 교환"처럼 session 을 서로 맞바꿔야 하는 경우도 이 1회의 계산 안에서 이미 반영되므로 교환은 별도 단계가 아닙니다. 어떤 배정으로도 조건을 충족할 수 없으면 1명씩 제외해 보아 그 사람을 제외하면 배정이 가능해지는 경우를 찾아 "이 인원을 제외하면 됩니다"라는 제안을 생성합니다. 이 제안을 [LLM 도우미](../llm-assistant/README.md)가 조율안 확정 항목을 가진 사람에게 전달하고, 그 사람이 승인하면 확정하고 화면을 갱신합니다.

불가능 시간은 어느 경우에도 무시되지 않습니다. "인원 제외"는 그 인원의 불가능을 무시하는 것이 아니라, 사람이 검토해 결정하는 선택지로만 제시됩니다.

### 잘못된 입력은 계산 전에 거절합니다

설정이 잘못된 채로 계산이 실행되면 잘못된 배정안이 정답처럼 확정되어 사람에게 전달됩니다. 특히 위험한 경우는 명단 오류가 "이 사람을 제외하라"는 조율안으로 출력되는 경우여서, 다음은 계산에 들어가기 전에 거절합니다.

| 거절하는 입력 | 거절하지 않으면 발생하는 일 |
| --- | --- |
| 같은 합주실의 같은 칸이 2회 세어짐 | 칸 1개가 2개로 처리되어 같은 합주실·같은 시간에 팀 2개가 배정됨 |
| 팀 번호가 중복됨 | 팀 1개가 결과에서 전부 삭제됨 |
| 팀 1개 명단에 같은 사람이 2회 | 배정 전체가 실패하고, 문제없는 사람을 제외하라는 제안이 출력됨 |
| 멤버가 없는 팀 | 아무도 없는 팀이 모든 시간에 "가능"으로 처리됨 |
| 팀당 session 횟수가 음수 | 호출하는 쪽의 실수가 "일정이 맞지 않는다"로 출력됨 |
| 운영 시간이 칸 격자를 벗어남 | 존재하지 않는 칸이 생성되거나 칸 크기 미만의 나머지 시간이 error 없이 삭제됨 |
| session 길이가 칸 크기의 배수가 아님 | session 의 끝이 칸 경계를 벗어나, 칸 단위로 판정하는 충돌 검사가 겹침을 놓침 |
| tzinfo(시간대 정보)가 있는 시각 | 같은 시각이 서로 다른 시각으로 처리되어 중복 방지가 전부 무효화됨 |

시간대는 현재 지원하지 않습니다. 지원하려면 여름시간제까지 함께 다뤄야 하므로 부분적으로 허용하는 대신 명시적으로 거절하기로 했습니다.

이 계산은 [기간 자동 배정](../scheduling-api/period-assignment/README.md) endpoint 가 저장된 팀·합주실·불가능 시간을 읽어 호출합니다. 그 endpoint 는 위 표의 검증 규칙을 다시 구현하지 않고, 계산이 거절한 입력은 그대로 거절된 결과로 반환합니다. 이름을 직접 입력해 보내던 이전 endpoint(`POST /assign`)는 2026-09-08 에 제거했습니다. 그 endpoint 를 호출할 수 있는 사람의 조건은 아래에서 다룹니다.
...
### 계산 1회 전체에 5분 상한을 설정했습니다 (2026-09-16)

배정을 찾지 못하면 멤버를 1명씩 제외한 계산을 멤버 수만큼 반복합니다. 반복 1회에 위의 solver 시간 상한이 각각 적용되므로, 전체 실행 시간은 멤버 수에 비례해 늘어납니다. 100명이면 계산 1회가 100시간까지 갈 수 있다는 뜻입니다.

그래서 작업(job) 1개의 전체 실행 시간에 상한 5분을 두었습니다. 넘기면 그때까지 찾은 조율안을 버리고 작업을 실패로 끝냅니다. 조율안을 남기지 않는 이유는 목록이 잘린 조율안을 "이것뿐"으로 읽으면 사람이 잘못 판단하기 때문입니다.

중단 여부는 반복과 반복 사이에서만 확인합니다. solver 계산 1회는 중간에 멈출 수 없으므로, 실제 중단 시각은 상한을 계산 1회(최대 60초)만큼 넘길 수 있습니다.

| 항목 | 값 | 위치 |
|---|---|---|
| 계산 1회(solver 1번)의 상한 | 60초 | `scheduling/pipeline.py` 의 `SOLVER_TIME_LIMIT_SECONDS` |
| 작업 1개 전체의 상한 | 300초 | `scheduling/pipeline.py` 의 `RESOLUTION_TIME_LIMIT_SECONDS` |
| 화면이 결과를 기다리는 상한 | 390초 | `frontend/src/lib/jobs.ts` 의 `JOB_DEADLINE_MS` |

화면의 상한은 서버가 작업 1개에 쓰는 최대 시간(300초 + 계산 1회 60초 = 360초)에 여유 30초를 더한 값입니다. 화면이 서버보다 먼저 대기를 중단하면 화면은 실패를 표시하고 서버는 계산을 계속해 저장하므로, 실패로 표시된 배정이 나중에 반영됩니다. 서버의 두 값을 변경하면 `jobs.ts` 의 `SERVER_JOB_LIMIT_SECONDS` 도 함께 변경합니다.

사람이 버튼을 눌러 실행한 경우 화면은 다른 실패와 같은 자리에 "배정 계산이 시간 상한을 넘어 중단했습니다" 를 표시합니다. 자동 실행의 경우 그 기간만 실패로 기록하고 다음 기간으로 넘어갑니다. 실행 기록을 남기지 않으므로 그 계산 시각은 다음 확인에서 다시 실행됩니다 — 상한을 넘기는 입력은 확인마다 5분을 다시 소모합니다.

검증: `backend/tests/unit/test_resolution.py::test_fails_when_the_proposal_search_exceeds_the_time_limit` 이 상한을 0초로 바꾸고 배정 불가능한 입력을 넣어 ValueError 를 확인합니다. 실행은 `docker compose run --rm --no-deps dev pytest -q tests/unit/test_resolution.py` 입니다.
...
자동 실행은 등록된 팀 전부와 합주실 전부를 대상으로 합니다. 사람이 버튼을 누를 때는 화면이 어느 팀과 어느 합주실로 계산할지 선택해 보내지만 자동 실행에는 선택할 사람이 없어서 지금은 등록된 팀과 합주실을 전부 사용합니다. 등록된 팀과 합주실 가운데 지정한 것만 계산하려면 "이 기간은 이 합주실만 사용한다"를 저장하는 table이 먼저 있어야 하고, 그 table이 없는 지금은 오늘이 기간 안에 드는 집중 합주기간이 2개 이상이면 서로 같은 칸을 배정하려다 1개가 실패로 기록됩니다.
...
### 배정은 구간 한 행으로 저장됩니다 (2026-09-16)

**계산은 session 하나씩 내놓고, 저장할 때 같은 팀이 같은 합주실에서 이어 쓰는 구간을 한 행으로 합칩니다**(`backend/src/backend/db/schedule_store.py` 의 `merge_runs`). 점유 단위를 설정에서 변경할 수 있게 되면서 5분 단위로 두면 합주실 하나의 하루치가 수백 행이 되기 때문입니다. 떨어진 구간은 합치지 않습니다. 합치면 배정받지 않은 시간까지 점유한 것이 됩니다.

2026-09-20 에 배정 단위가 칸에서 session 으로 바뀌면서, 합쳐지기 전의 행이 이미 session 1회입니다. 18:00~19:00 과 19:00~20:00 을 같은 팀이 연달아 받으면 저장은 18:00~20:00 한 행입니다.

같은 합주실에서 시간이 겹치는 행은 DB 가 직접 거절합니다(`assignments_no_overlap`). 예약과 같은 방식이고, 이전의 `(합주실, 시작 시각)` 중복 금지를 대신합니다. 시작 시각만 비교하면 길이가 다른 두 구간이 겹쳐도 통과하기 때문입니다.

이미 저장되어 있던 행은 합치지 않았습니다. 배정은 계산할 때마다 그 기간의 행을 전부 지우고 다시 쓰므로 다음 계산에서 합쳐지고, 그때까지 칸마다 한 행으로 남아 있어도 서로 겹치지 않아 새 제약에 걸리지 않습니다.

되돌리기 목록에 표시하는 칸 수는 행 수가 아니라 구간의 길이를 칸 크기로 나눈 값입니다. 행을 세면 같은 배정이 점유 단위에 따라 다른 수로 보입니다.

**어떻게 확인했나.** 이어진 칸이 구간 한 행이 되는지, 떨어진 칸은 두 행으로 남는지, 이어진 칸이라도 팀이 다르면 각각의 행인지를 `backend/tests/integration/db/test_schedule_store.py` 에 담았습니다. 되돌리기가 합쳐진 구간을 그대로 복원하는지는 `backend/tests/integration/db/test_period_endpoints.py` 의 `test_rollback_restores_the_previous_schedule` 이 시각까지 값으로 비교합니다. 마이그레이션은 `alembic upgrade head` 와 `alembic downgrade -1` 을 실제로 실행해 확인했습니다.
...
flowchart TD
    ...
    S -->|그렇다| M[그 기간에 배정을 받은 팀을 저장된 시간표에서 조회합니다]
...
알림은 그 기간에 실제로 배정을 받은 팀의 사람에게만 저장합니다.

| 누구 | 저장되는가 | 왜 |
| --- | --- | --- |
| 배정을 받은 팀의 소속 | 저장됩니다 | 그 사람의 시간표가 실제로 변경되었습니다 |
| 배정을 못 받은 팀의 소속 | 저장되지 않습니다 | 시간표가 그대로라 알릴 내용이 없습니다 |
...
대상은 계산에 넘긴 팀 목록이 아니라 저장된 시간표에서 조회한 팀입니다. 넘긴 목록을 쓰면 배정을 못 받은 팀에게도 알림이 저장됩니다.

**읽음 처리는 행을 삭제합니다**(2026-09-19). 이전에는 `read_at` 에 시각을 설정하고 행을 보존했는데, 행을 삭제하는 경로가 코드에 없어 알림이 쌓이기만 했습니다. 알림은 배정 1회마다 배정받은 팀의 멤버 전원에게 1행씩 생성되고, 조회는 상한 없이 전 행을 반환하며 화면은 20초마다 다시 조회합니다. 운영 기간에 비례해 전송량이 증가하는 구조였습니다.

읽은 알림을 다시 볼 경로가 없으므로 보존하지 않습니다. 목록에 남아 있는 알림은 전부 읽지 않은 알림이고, `read_at` 열은 migration `9b2f5d81ac34` 로 삭제했습니다. 보관 기간과 조회 개수 상한은 두지 않습니다(사용자 결정).
...
확인은 저장에 성공했을 때만 저장되는지, 배정을 받은 팀의 사람에게만 저장되는지, 팀이 없는 사람과 승인 대기 중인 사람은 제외되는지, 2개 이상 팀에 속한 사람에게 1행만 저장되는지, 읽음 처리가 요청자의 알림만 삭제하고 다른 사람의 알림은 남기는지를 테스트에 담아서 했습니다. 그 테스트에 사람이 버튼을 눌러 다시 계산해도 알림이 저장되는지와 되돌리기에도 알림이 저장되는지 2가지를 추가했습니다. 테스트 파일은 맨 위 목록의 알림 시나리오 파일 1개이고, 서버 테스트 전체 468개가 통과하며 타입 검사는 94개 파일에서 이상이 없습니다.
...
### 배정 단위를 칸에서 session 으로 변경했습니다 (2026-09-20)

**한 팀이 받는 1회는 session_minutes 동안 끊기지 않는 구간 1개입니다.** 이전에는 칸 1개가 배정의 단위여서 칸 크기가 곧 합주 길이였고, 30분 칸으로 설정하면 계산이 18:00·20:30·22:00 처럼 떨어진 칸을 한 팀에게 나눠 주는 것이 정상 동작이었습니다.

| | 이전 | 지금 |
| --- | --- | --- |
| 배정의 단위 | 칸 1개 | session 1회(칸의 배수 길이) |
| 시작할 수 있는 시각 | 칸의 경계 | 칸의 경계(동일) |
| 팀마다 같은 것 | 칸 개수 | session 횟수 |
| 결과 1항목의 길이 | 칸 크기 | session 길이 |

충돌은 session 끼리 비교하지 않고 session 이 차지하는 칸으로 판정합니다. 18:00 시작과 18:30 시작은 서로 다른 후보지만 18:30~19:00 칸을 함께 쓰므로, 후보만 비교하면 겹침을 놓칩니다.

session 1회를 채우지 못하고 남는 칸은 자동 배정 대상에서 빠지고 빈 자리 목록에 남습니다. 합주실이 18~19:30 을 열고 session 이 60분이면 19:00~19:30 이 그 칸입니다.

**어떻게 확인했나.** 30분 칸에 60분 session 일 때 결과 1항목이 60분 구간인지, 같은 팀의 2회가 겹치지 않는지, 두 팀이 30분만 겹치는 것도 거절되는지, session 이 차지한 칸이 빈 자리 목록에서 빠지는지를 계산 단위 테스트에 담았습니다. endpoint 까지 거친 결과가 60분 구간 4개인지는 기간 배정 시나리오 테스트가 확인합니다. 서버 테스트 전체 650개가 통과하고 타입 검사는 105개 파일에서 이상이 없습니다.

### 조절 값을 기능 파일에서 분리했습니다 (2026-09-20)

칸 크기·합주 1회 길이·solver 상한 2개를 계산 코드가 들고 있었습니다. 계산 코드를 수정하지 않고 값만 변경하려는 사람이 그 값을 계산 코드 안에서 찾아야 했고, 같은 값을 참조하는 db·api·services 가 계산 코드를 직접 참조하게 됩니다.

값을 성격에 따라 두 곳으로 분리했습니다.

| 값 | 어디에 있나 | 왜 그 자리인가 |
| --- | --- | --- |
| 선택 가능한 칸 크기, 칸 크기 기본값, 합주 1회 길이 기본값과 상한 | 공유 규격 파일(`backend/contract.py`) | db·api·services·scheduling 4개 모듈이 같은 값을 참조합니다. 선언만 두고 계산을 두지 않습니다 |
| solver 계산 1회의 상한, 작업 1개 전체의 상한 | 모듈 진입점(`scheduling/pipeline.py`) | scheduling 안에서만 씁니다. 계산 1회의 상한과 작업 전체의 상한을 한 자리에서 비교할 수 있어야 둘의 비율을 판단할 수 있습니다 |

계산 파일 3개(칸 분할·배정·조율안)에는 상수가 1개도 남지 않았습니다. 필요한 값은 전부 매개변수로 받고, 진입점이 호출할 때 전달합니다.
...
자동 배정은 해결 가능한 경우는 자동으로 확정하고, 불가능한 경우만 "누구를 제외하면 되는지" 제안으로 사람에게 제시합니다. 잘못된 설정은 계산 전에 거절해 오류 있는 배정이 확정되는 일을 방지합니다. 이제는 실행 시작까지 자동화되었습니다. 정해진 시각이 지나면 아무도 개입하지 않아도 계산이 실행되고, 시간표가 실제로 변경된 사람에게는 그 사실이 알림으로 전달됩니다. 모바일 앱 푸시 알림만 미구현으로 남아 있습니다. 생성된 제안을 사람에게 전달하는 방법은 [LLM 도우미](../llm-assistant/README.md)에서 계속됩니다.
