...
flowchart TD
    ...
    IMG -->|그렇다| BUILD[개발용 이미지를 다시 빌드합니다]
    IMG -->|아니다| MIG
    BUILD --> MIG[table 을 최신 revision 으로 맞춥니다]
    MIG --> UP[API 와 화면을 시작합니다]
    UP --> WAIT[2곳에 실제로 요청해<br/>응답할 때까지 대기합니다]
    WAIT -->|응답함| HINT[가입된 계정이 있는지 안내하고<br/>브라우저를 엽니다]
    WAIT -->|시간 초과| FAIL[확인할 위치를 안내합니다]
```

## Abstract

개발 환경 실행은 저장소를 받은 사람이 명령 1개로 동작하는 시스템에 도달하게 하는 기능입니다. 이미지(Docker container를 실행하는 템플릿)를 다시 빌드할지, 마이그레이션(데이터베이스 구조 변경)을 먼저 실행할지, 서비스가 실제로 요청에 응답하는지를 판단하는 모든 작업을 코드로 자동화했습니다. 가장 중요한 판단은 container(Docker가 실행하는 격리된 프로세스 단위) 프로세스가 실행 중인 상태와 애플리케이션이 실제로 요청에 응답하는 상태를 구분하는 판단인데, 이 기능은 container 상태가 아니라 실제 네트워크 응답을 근거로 준비 여부를 확인합니다.

## Introduction

이 시스템은 API 서버, 화면 서버, 자동 배정 계산 서비스, 그리고 데이터베이스 4개 서비스로 구성되며, 모든 실행이 container 내에서 일어납니다. 따라서 개발 환경을 준비하려면 순서가 정해진 4단계를 거쳐야 합니다.

문제는 순서를 지키지 않았을 때의 실패가 드러나지 않는다는 점입니다. 마이그레이션보다 먼저 서비스를 실행하면 첫 조회에서 존재하지 않는 table을 조회합니다. 의존성을 추가한 후 이미지를 다시 빌드하지 않으면 애플리케이션이 import 단계에서 중단됩니다. 2가지 경우 모두 container 목록에는 정상으로 표시됩니다.
...
- [배포](../deployment/README.md) — 같은 구성에서 개발용 설정을 제거한 상태
...
- [자동 배정](../auto-assignment/README.md) — 선택적으로 함께 시작하는 계산 서비스
...
이미지 검사가 1번째인 이유는 이 단계가 실패했을 때 증상이 가장 드러나지 않기 때문입니다. container는 정상적으로 실행되고 목록에도 표시되는데 애플리케이션만 중단되어 있어서, 이후의 모든 단계가 정상으로 보이는 상태에서 실패합니다.

### 실행 중인 상태와 응답하는 상태는 다릅니다

container 목록은 프로세스가 실행 중인지만 표시합니다. 애플리케이션이 실제로 요청을 받을 수 있는지까지는 표시하지 않으므로, 이 기능은 2개 서비스에 실제 요청을 보내 응답할 때까지 대기합니다.
...
### 이미지가 오래되었는지는 수정 시각으로 판단합니다

이미지를 매번 다시 빌드하면 느리고, 빌드하지 않으면 error 없이 실패합니다. 따라서 의존성 파일, 잠금 파일, 빌드 정의의 수정 시각 3가지를 이미지 생성 시각과 비교하여, 하나라도 더 최신이면 다시 빌드합니다.
...
### 어느 경로에서나 호출할 수 있게 등록합니다

이 기능은 저장소 내 스크립트이지만, 등록하면 다른 경로에서도 이름만으로 호출할 수 있습니다. 이때 별칭이 아니라 함수로 등록하는 이유는 별칭은 이름만 변경할 뿐 뒤따르는 인자를 전달하지 못하는 경우가 있기 때문입니다. 등록 내용은 표시 2줄 사이에서만 작동하므로 2회 이상 실행해도 중복되지 않습니다.
