...
sources:
  ...
  - frontend/src/routes/ProfileCards.tsx         # 프로필 화면의 카드 — 사진·내 정보·비밀번호·테마·탈퇴
  - frontend/src/components/Avatar.tsx           # 사람 한 명을 나타내는 동그라미 — 사진이 없으면 이름 앞 두 글자
...
| `/settings` | 합주실 · 기간 · 멤버 · 예약 · 블라인드 탭. 가진 권한의 탭만 표시되고, 관리 권한이 없으면 사이드바에 메뉴가 없습니다 | 예. 조회하고, 생성하고, 수정합니다 |
...
이름과 기수를 고치는 입력칸이 설정 화면의 계정 탭과 프로필 화면 두 곳에 있었습니다. 프로필 화면은
읽기만 하고 수정은 설정 탭에서 하는 구조라, 프로필을 열어 본 사람이 다시 설정으로 옮겨 가야 했습니다.

계정 탭을 없애고 그 카드 전부를 프로필 화면으로 옮겼습니다. 설정 화면에는 **다른 사람·공용 자원을
다루는 탭만** 남습니다. 두 화면의 구분이 "내 것"과 "모두의 것"으로 정리됩니다.

| 화면 | 다루는 것 |
| --- | --- |
| `/profile` | 내 사진·이름·기수·비밀번호·화면 테마·회원 탈퇴 |
| `/settings` | 합주실·기간·멤버 권한·모든 예약·가려진 글 |

설정 화면에 남은 탭은 전부 권한이 필요합니다. 그래서 관리 권한이 하나도 없는 사람에게는 **사이드바에
설정 메뉴가 표시되지 않습니다.** 주소를 직접 입력해 들어오면 프로필 화면으로 가라는 안내가 나옵니다.

### 프로필 사진 (2026-09-23)

사람을 나타내는 동그라미에 사진을 표시합니다. 사진이 없으면 지금까지처럼 이름 앞 두 글자를 표시합니다.
사진이 있는지를 응답마다 싣지 않고, 사진 주소를 그대로 표시해 본 뒤 없으면(404) 글자로 바꿉니다 —
사람 이름을 표시하는 응답이 여러 개라, 응답마다 항목을 더하면 고칠 곳이 그만큼 늘어납니다.

사진을 표시하는 자리는 상단바 프로필 버튼, 프로필 카드, 프로필 화면 세 곳입니다. 달력과 날짜 모달에는
넣지 않았습니다 — 칸이 좁아 이름조차 줄여 표시하는 자리입니다(사용자 결정).

바꾼 사진이 바로 보이게 하려고 `/me` 응답이 사진의 저장 파일명을 함께 내려줍니다. 사진 주소는 계정
번호로 고정이라, 이 값이 없으면 브라우저가 옛 사진을 계속 표시합니다.
...
"타 멤버 글 삭제 및 블라인드"(당시 이름은 "타 멤버 글 수정 및 삭제")와 "타 멤버 예약 수정 및 취소"는 서버가 이미 지원하고 있었지만 화면 어느 곳에서도 구현되지 않아, 권한을 부여받아도 화면에서는 아무 동작이 일어나지 않았습니다.

| 항목 | 화면의 동작 |
| --- | --- |
| 타 멤버 글 삭제 및 블라인드 | 공지와 팀 게시판에서 다른 사용자의 글·댓글·첨부 옆에도 **삭제** 버튼이, 글에는 **블라인드** 버튼이 표시됩니다. 수정 버튼은 작성자에게만 표시됩니다. 2026-09-16부터 서버도 남의 글 수정을 거절합니다. 가려 둔 글은 설정 화면의 블라인드 탭에서 보고 되돌립니다 |
...
현재는 목록 아래에 **`+ 새 ~~` 버튼 1개**만 있고, 클릭하면 dialog(화면 위에 뜨는 대화 상자)가 열립니다. 사용자를 검색해야 하는 경우 dialog 위에 다른 dialog를 중첩하여 표시합니다.

팀 창은 2026-09-15 에 2단계(구성 설정 → 멤버 지정)를 **창 1개**로 통합했습니다(patch_note 11번). 새 팀과 팀 수정이 같은 창이고, 수정 창에는 목록의 삭제 아이콘과 같은 동작인 팀 삭제 버튼이 있습니다. 창 안에서 멤버를 지정·삭제해도 서버에는 저장 버튼을 누를 때 한 번에 전송합니다.
