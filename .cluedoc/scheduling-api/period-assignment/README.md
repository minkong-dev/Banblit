...
- **어떻게 해결했는지**: 저장 중 이 제약 위반이 나면 그 자리에서 되돌리고(롤백), "다른 기간이 이미 같은 합주실의 같은 시간을 사용하고 있습니다"는 문구와 함께 사용자 오류로 변경해 응답합니다. 같은 김에 요청에 같은 팀·합주실 번호가 중복으로 들어오면 조용히 하나로 합쳐지던 것도 미리 거부하도록 함께 정리했습니다
