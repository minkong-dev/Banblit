...
sources:
  ...
  - backend/src/backend/api/notification_service.py  # 시간표를 바꾼 통로가 뒤이어 알림을 남기는 자리
---
...
**통로마다 부를 수 있는 사람이 다르다.** 예전에는 로그인하지 않은 사람도 배정 계산을 시킬 수 있었다. 지금은 상태 확인 하나를 뺀 모든 통로가 "요청한 사람이 누구인가"를 먼저 확인하고, 그 확인은 한 자리에 모여 있다 — 통로마다 쿠키를 따로 들여다보면 한 곳만 빠뜨려도 그 통로가 통째로 열린다. 누구인지 가려내는 방법 자체는 [계정과 역할](../accounts-and-roles/README.md)에서 다룬다.

| 통로 | 부를 수 있는 사람 | 그렇게 정한 이유 |
| --- | --- | --- |
| 상태 확인 | 누구나 — 로그인하지 않아도 된다 | 부르는 쪽(배포 도구, 앞단)이 계정을 가질 수 없다 |
| 배정 요청 (저장하지 않는 계산) | 로그인한 사람 전체 | 저장은 하지 않지만 계산이 최대 22.2초까지 걸린 실측이 있다(2026-08-28). 로그인 없이 열어 두면 이 통로 하나로 서버를 묶을 수 있다 |
...
표의 아래 다섯 줄은 저장된 기간을 다루는 통로다. 그 동작 자체는 [기간 자동 배정](./period-assignment/README.md)과 [자동 배정](../auto-assignment/README.md)에서 다루고, 여기 함께 적은 것은 규칙을 한 장에서 비교해 보기 위해서다. 게시판·예약처럼 다른 문서가 다루는 통로의 규칙은 각자의 문서에 있다.

**계산 결과를 되묻는 자리도 함께 잠근다.** 배정을 시키는 쪽만 잠그고 결과를 읽는 쪽을 열어 두면, 막아 둔 것을 옆문으로 그대로 내주는 셈이 된다. 다만 두 자리가 요구하는 것은 **서로 다른 항목**이다 — 계산은 못 돌리지만 남이 돌린 결과를 검토만 하는 사람을 둘 수 있다. 항목이 무엇이고 어떻게 켜고 끄는지는 [계정과 역할](../accounts-and-roles/README.md)이 정본이다.

**상태 확인은 열어 두었지만 지금은 아무도 부르지 않는다.** 컨테이너 구성의 상태 확인은 데이터베이스 컨테이너에만 붙어 있다. 앞단이나 배포 도구가 이 서버의 상태를 물어보기 시작할 때를 위해 열어 둔 상태다.

**시간표를 바꾼 통로는 뒤이어 알림을 남긴다.** 위 표에서 시간표를 실제로 바꾸는 세 자리(배정 시키기·조율안 확정·되돌리기)는, 저장이 **실제로 된 때만** 그 기간에 자리를 받은 팀의 사람에게 알림 한 줄을 남긴다. 저장하지 않는 계산과 조회는 아무것도 남기지 않는다. 이 계층이 새로 정하는 규칙은 아니고, 누구에게 무엇이 남는지는 [자동 배정](../auto-assignment/README.md)이 정본이다.

**401과 403을 가른다.** 로그인하지 않은 것과, 로그인했지만 자격이 없는 것은 사람이 해야 할 일이 다르다. 앞은 로그인 화면으로 보내야 할 사람이고, 뒤는 보내봐야 같은 자리로 되돌아온다.
...
**저장소에 닿지 못하면 "지금은 못 받는다"고 답한다.** 다섯 종류의 실패를 화면이 구별할 수 있어야 한다.

| 상황 | 응답 | 화면이 해야 할 일 |
| --- | --- | --- |
| 요청이 잘못됨 | 422 | 무엇이 잘못됐는지 사용자에게 보여준다 |
| 로그인하지 않음 | 401 | 로그인 화면으로 보낸다 |
| 로그인했지만 자격이 없음 | 403 | 할 수 없는 일이라고 알린다. 로그인 화면으로 보내지 않는다 |
...
이 명령으로 위 네 파일을 포함한 전체 486개 검사가 통과함을 확인했다. 아무것도 띄우지 않고 순수 계산 부분만 확인하려면 `docker compose run --rm --no-deps dev pytest tests/unit -q`로 범위를 좁힐 수 있다.

타입 표기는 따로 확인한다. 통로마다 걸어 둔 확인 자리가 실제로 사람 하나를 돌려주는지는 표기로만 드러나기 때문이다.

```
docker compose run --rm --no-deps dev mypy
```

104개 파일에서 이상이 없음을 확인했다.
