...
- [화면 프로토타입](../screen-prototypes/README.md) — 이 통로를 실제로 부르는 화면들. 개발 환경에서는 이 서버가 그 화면들을 함께 내보낸다
...
**개발할 때만 화면을 함께 내보낸다.** 브라우저는 화면을 받아온 곳과 다른 곳에 값을 물으면 막는다. 그래서 개발 환경에서는 이 서버가 API 통로 외에 **화면을 내보내는 자리 하나를 더** 갖는다. 화면과 API가 같은 곳에서 오면 화면이 이 서버에 자유롭게 물어볼 수 있다.
