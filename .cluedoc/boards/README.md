...
| 올리기 | 그 글의 글쓴이만 |
| 지우기 | 그 글의 글쓴이, 또는 "타 멤버 글 수정 및 삭제" 권한자. 화면도 그 두 사람에게만 지우는 자리를 보여준다(글·댓글의 삭제 버튼도 같다) |
...
글을 삭제하는 endpoint는 새로 추가되었고 그 글을 작성한 사람만 호출할 수 있습니다. 아직 화면에 버튼는 없고 server endpoint만 구현되어 있습니다. table도 하나 추가되었는데, 파일의 내용은 디스크에 있고 table에는 파일의 저장 위치·표시 이름·크기·파일 종류만 저장됩니다. 저장소에서 어떤 제약이 적용되어 있는지는 [데이터 저장소](../database-layer/README.md)에서 다룹니다.
