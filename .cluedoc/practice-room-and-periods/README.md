...
title: 합주실과 기간
sources:
  - backend/src/backend/scheduling/slots.py       # 운영 시간을 slot으로 분할합니다
...
slot 길이는 설정의 점유 단위(`settings.slot_minutes`, 5·10·12·15·20·30·60분 중 하나)입니다. 설정값을 넘기지 않은 호출의 기본값은 `backend/contract.py` 의 `DEFAULT_SLOT_MINUTES`(60분) 상수 1개로 정의합니다. 개방/폐쇄 시간 검증 규칙도 점유 단위에서 도출됩니다.
...
자동 배정은 2026-09-20 부터 이 길이를 단위로 자리를 배치합니다. 팀 하나가 받는 1회는 이 길이만큼 끊기지 않는 구간 1개이고, 충돌 판정은 그 구간이 차지하는 칸으로 합니다 — [자동 배정](../auto-assignment/README.md)에 있습니다. 선착순 예약은 이 길이와 무관하게 칸 단위 그대로입니다.
...
**실제 배정 구간은 시간대와 합주실 개방시각의 교집합입니다.** 17~23시로 정해도 합주실이 22시에 닫으면 22시까지입니다. 겹치는 시간이 없는 날은 배정 대상에서 빠집니다 — 길이가 0 이거나 음수인 구간을 엔진에 넘기면 칸을 만들지 못하거나 운영 시간이 잘못되었다고 거부합니다.

**시간대 밖의 시간도 예약은 열려 있습니다.** 자동 배정만 하지 않을 뿐입니다. 남는 칸을 계산하는 곳(`open_slots_in_period`)은 시간대를 넘기지 않고 개방시각 전체를 씁니다.

정하지 않은 쌍은 두 열 모두 null 이고, 그날은 지금처럼 개방시각 전체를 씁니다. 한쪽만 채우면 끝 시각 없는 시간대가 되어 배정 구간을 정할 수 없으므로 DB CHECK(`periods_practice_window_pairs`)가 거절합니다. 시각이 점유 단위 격자 위인지는 화면(`lib/settings.ts` 의 `practiceWindowMessage`)이 먼저 봅니다 — 격자를 벗어나면 저장은 되고 배정만 실패해 원인을 찾기 어렵습니다.

### 기간은 2종류입니다

| 언제 | 사용 방식 | 자동 배정 | 등록이 필요한가 |
| --- | --- | --- | --- |
| 집중 합주기간으로 덮인 날 | 팀마다 같은 횟수의 합주 1회를 배정 | 있음 | 집중 합주기간을 등록합니다 |
...
집중 합주기간에는 모든 팀이 정확히 같은 횟수의 합주 1회를 할당받습니다. 예를 들어 저녁 6시간이 2주간 개방되고 팀이 6개이며 합주 1회가 1시간이면, 각 팀은 14회를 배분받습니다. 팀 1개라도 그 횟수를 채우지 못하면 전체 배정이 실패로 처리되어 조율안 확정 항목을 가진 사람이 조율안을 결정합니다.
