...
여기에 비밀번호 추측 공격을 방어하는 rate limit(일정 시간 안의 요청 횟수 제한)이 로그인 관련 5개 endpoint(API의 요청 주소 단위)에 적용되어 있습니다.
...
요청 출처는 **앞단이 관찰한 IP 주소**로 식별합니다. 요청을 보낸 쪽이 제출한 값은 신뢰하지 않습니다. 신뢰할 경우 요청자가 IP를 변경하며 시도하여 임계값을 무한히 회피할 수 있습니다. 앞단이 2단 이상이 되면 이 식별 방식도 함께 검토해야 합니다.
